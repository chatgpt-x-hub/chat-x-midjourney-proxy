...
namespace App\Midjourney;

use Jenssegers\Agent\Agent;
use Throwable;
use App\Midjourney\MessageHandler\DescribeSuccess;
use App\Midjourney\MessageHandler\Error;
use App\Midjourney\MessageHandler\InteractionFailure;
use App\Midjourney\MessageHandler\ModalCreateStart;
use App\Midjourney\MessageHandler\Progress;
use App\Midjourney\MessageHandler\Start;
use App\Midjourney\MessageHandler\Success;
use App\Midjourney\MessageHandler\UpscaleSuccess;
use App\Midjourney\MessageHandler\VaryRegionStart;
...
class Discord
{
    ...
    public function createWss()
    {
        ...
        (new \Ratchet\Client\Connector(
            null,
            $connector
        ))("$gateway?encoding=json&v=9&compress=zlib-stream", [], [
            ...
            $conn->on('message', function($data) use ($conn) {
                ...
                if (in_array($data['t'] ?? '', [Discord::INTERACTION_MODAL_CREATE, Discord::INTERACTION_IFRAME_MODAL_CREATE]) && !in_array($data['d']['title'] ?? '', $titleWhiteList)) {
                    Log::channel('midjourney')->warning($data['d']['title'] ?? '');
                }
                switch ($code) {
    ...
    {
        return Config::get('midjourney.proxy.server') ?? static::SERVER_URL;
    }

    public static function getGateway()
    {
        return Config::get('midjourney.proxy.gateway') ?? static::GATEWAY_URL;
    }
    ...
    {
        $cdn = Config::get('midjourney.proxy.cdn');
    ...
    {
        $cdn = Config::get('midjourney.proxy.upload');
