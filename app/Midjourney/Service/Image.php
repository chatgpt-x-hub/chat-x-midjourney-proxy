...
namespace App\Midjourney\Service;
...
class Image
{
    ...
    public static function imagine(Task $task, Discord $discord)
    {
        ...
        $client = new Client();
        $client->request(trim(Config::get('midjourney.proxy.server'), ' /') . '/api/v9/interactions', static::payloadJson($task, $discord, $params));
    }
    ...
    public static function change(Task $task, Discord $discord)
    {
        $params = static::getParams($task, $discord);
        $client = new Client();
        $client->request(trim(Config::get('midjourney.proxy.server'), ' /') . '/api/v9/interactions', static::json($task, $discord, $params));
    }
    ...
    public static function varyRegion(Task $task, Discord $discord)
    {
        ...
        if (!($task->params()[Discord::INTERACTION_IFRAME_MODAL_CREATE] ?? '')) {
            $url = trim(Config::get('midjourney.proxy.server'), ' /') . '/api/v9/interactions';
    ...
    public static function cancelJob(Task $task, Discord $discord)
    {
        $params = static::getParams($task, $discord);
        $client = new Client();
        $client->request(trim(Config::get('midjourney.proxy.server'), ' /') . '/api/v9/interactions', [
            'method' => 'POST',
            'headers' => [
                'Authorization' => $discord->token(),
                'Content-Type' => 'application/json',
